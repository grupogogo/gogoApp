...
export const AreaChartOldYearAcumulada = ({ data }) => {
  ...
  const buildLineDatasets = (rawDatasets, number_format, visibilityState) =>
    rawDatasets.map((dataset, index) => {
      // Acumulado sin diferir el primer mes
      const accumulatedData = dataset.data.reduce((acc, val, i) => {
        if (i === 0) {
          acc.push(val); // Primer mes inicia con su propio valor
        } else {
          acc.push(val + acc[i - 1]); // Suma acumulada
        }
        return acc;
      }, []);
