...
export const Ventas = ({ datosUsuario }) => {
    ...
    useEffect(() => {
        ...
        if (gastos.length > 0) {
            const gastosTotales = totalGastosUsuarios(gastos, filtroAnio, datosUsuario);
            updateDashboardState({
                gastosTotales
            });
        }
...
return (
  <>
    <Estadisticas totales={dashboardState.totales} tipo={1} gastoUsuarios={''} />
    ...
    <div className="row g-3 mb-4">    
        ...
        <div className="col-xl-6 col-lg-4 col-md-12">
          <div className="card card-hover border-0 shadow-lg">
            <div className="d-flex justify-content-between align-items-center">
            <h6 className="mb-0 text-gray m-3">Gastos Kits</h6>
            </div>
            <div className="card-body p-1">
            <PieChart
              arrayLabels={['TOTAL KITS', 'OG', 'LG']}
              arrayData={dashboardState.gastosTotales}
              title={`GASTOS KITS ${formatearPrecio(dashboardState.gastosTotales[0])}`}
            />
            </div>
          </div>
        </div>
